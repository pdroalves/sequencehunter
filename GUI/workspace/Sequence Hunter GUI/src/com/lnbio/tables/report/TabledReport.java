...
import com.lnbio.auxiliares.WaitLayerUI;
import com.lnbio.database.DBManager;
import com.lnbio.histogram.EventHistogram;
import com.lnbio.histogram.ReportHistogramPanel;
...
public class TabledReport extends Report implements Observer{
		...
		jsb.addAdjustmentListener(new AdjustmentListener(){
			...
			public void adjustmentValueChanged(AdjustmentEvent e) {
				...
				if(jsbMax*0.6 <= jsbPos){
					...
					if(getName().contains("central"))
						jrtm.centralCutLoad();
					else if(getName().contains("five"))
						jrtm.fiveCutLoad();
					else if(getName().contains("custom"))
						if(jrtm instanceof JFiveCutQueryReportTableModel)
							((JFiveCutQueryReportTableModel)jrtm).customHuntLoad();
				}
			}					
		});
	...
	public void update(Observable o, Object arg) {
		dbm = (DBManager)arg;
		if(dbm.isReady()){
			System.err.println("Atualizando - "+getName()+" - size:"+dbm.getCentralCutEvents().size());
			//eh.clearAllBars();
			switch(jrtm.getKind()){
			case JReportTableModel.CENTRAL_CUT:
				dbm.normalizeCentralCutData();
