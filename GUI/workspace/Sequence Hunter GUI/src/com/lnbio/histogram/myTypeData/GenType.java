package com.lnbio.histogram.myTypeData;

import java.awt.Color;

import com.lnbio.histogram.HistogramBar;

public class GenType extends HistogramBar{
	private String seq;
	private int freq;
	private float relativeFreq;
	
	public GenType(String s){
		super();
		seq = s;
		freq = 0;
		relativeFreq = 0;
	}
	
	public GenType(String s,Color c){
		super();
		seq = s;
		freq = 0;
		relativeFreq = 0;
		super.setColor(c);
	}

	public GenType(String s,int f){
		super();
		seq = s;
		freq = f;
		relativeFreq = 0;
	}

	public GenType(String s,int f,Color c){
		super();
		seq = s;
		freq = f;
		relativeFreq = 0;
		super.setColor(c);
	}
	
	public String getTag(){
		return seq;
	}
	
	public void setTag(String s){
		seq = s;
	}
	
	public int getFreq(){
		return freq;
	}
	
	public void setFreq(int n){
		freq = n;
	}
	
	public float getRelativeFreq(){
		return relativeFreq;
	}

	public void setRelativeFreq(float r){
		relativeFreq = r;
	}
	
	public void setBar(int x,int y,int width,int height){
		super.setX(x);
		super.setY(y);
		super.setWidth(width);
		super.setHeight(height);
	}
	
	public void setColor(Color c){
		super.setColor(c);
	}
	
	public Color getColor(){
		return super.getColor();
	}
	
	// Metodos override de HistogramBar
	
	@Override
	public String getName(){
		return getTag();
	}
	
	@Override
	public void setName(String n) {
		setTag(n);
	}

	@Override
	public void setValue(int n) {
		setFreq(n);
	}

	@Override
	public int getValue() {
		return getFreq();
	}
	
}
